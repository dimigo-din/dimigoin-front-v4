...
export const getFrigo = async (): Promise<Frigo> => {
  return (await client.get("/student/frigo")).data;
}

export const applyFrigo = async (timing: string, reason: string): Promise<Frigo> => {
  return (await client.post("/student/frigo", { timing: timing, reason: reason, grade: localStorage.getItem("grade") })).data;
}

export const deleteFrigo = async (): Promise<Frigo> => {
  return (await client.delete("/student/frigo")).data;
}